from flask import Flask, request, jsonify, render_template, redirect, url_for, flash, session, url_for
import torch
...
def predict():
    ...
    if file:
        ...
        with torch.no_grad():
            ...
            accuracy = float(torch.max(probabilities) * 100)

            # Only proceed if accuracy is 90% or higher
            if accuracy >= 90:
                predicted_blood_group = torch.argmax(prediction).item()
                blood_groups = ['A+', 'A-', 'AB+', 'AB-', 'B+', 'B-', 'O+', 'O-']
                predicted_group_name = blood_groups[predicted_blood_group]
                
                # Store prediction data in session for report generation
                session['prediction_data'] = {
                    'blood_group': predicted_group_name,
                ...
                os.remove(file_path)
                return render_template('result.html', blood_group=predicted_group_name, accuracy=round(accuracy, 2))
            else:
                # If accuracy is too low, ask user to retake the fingerprint
                os.remove(file_path)
                return render_template('low_accuracy.html', accuracy=round(accuracy, 2))
...
def predict_blood_group():
    if 'user_id' not in session:
        ...
        return redirect(url_for('login'))
    return render_template('predict_blood_group.html')
...
def generate_report():
    ...
    if not prediction_data:
        flash('No prediction data found. Please perform a prediction first.', 'error')
        return redirect(url_for('predict_blood_group'))

    # Render the report template with all data
    return render_template('report.html',
                         fullname=prediction_data.get('name', user['fullname']),
                         email=user['email'],
                         age=prediction_data.get('age'),
                         gender=prediction_data.get('gender'),
                         contact=prediction_data.get('contact'),
                         blood_group=prediction_data['blood_group'],
